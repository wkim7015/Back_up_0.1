...
class TESTmake_report_open_report(unittest.TestCase):
    ...
    def test_existence(self):# worked.
        ...
        file_check1  = os.path.isfile(unicode_conversion(self.path3))
    ...
    def test_Capital_letter(self):
        ...
        s = check_file(unicode_conversion(self.capitalString))
        self.assertTrue(s,msg = "True")
...
class TESTlog(unittest.TestCase):
      ...
      def test_initial(self):
          ...
          os.remove(unicode_conversion(self.path))
          self.text.initial(self.path) #manually checked
          lines = [line.rstrip('') for line in open(unicode_conversion(self.path))]
          check_lines = [unicode_conversion(str(date.today())+"\t\t\t에러 리포트\n"),"\n","\t\n",
                       unicode_conversion("사용한 텍스트 파일 주소:\t")+self.path+"\n","\n",unicode_conversion("로그 시작한 시간:\t\t "+str(datetime.now().replace(second=0, microsecond=0))+"\n"),"\n","\n",
                       "\t\t" +unicode_conversion("\t백업 과정 보고서\n"),"\n","\n"]
      ...
      def test_success(self):
              ...
              check_lines = [unicode_conversion("성공여부\t\t**\t #!!복사 성공!!#\t:)\n".rstrip('\n')),unicode_conversion("코멘트\t\t\t\t")]
      ...
      def test_failed(self):
              ...
              lines = [line.rstrip('\n') for line in open(unicode_conversion(self.path))]
              check_lines = [unicode_conversion("성공여부\t\t**\t #!!복사 실패!!#".rstrip('\n')+ "\tX(\n".rstrip('\n')),unicode_conversion("코멘트\t\t\t\t")]
              self.assertEqual(lines,check_lines)
...
class TESTfinal(unittest.TestCase):
    ...
    def test_make_reportfile(self):
        ...
        for i in range(0, self.N):
             ...
             file_check1  = os.path.isfile(unicode_conversion(self.pathDirectory+"\\"+x+".txt"))
             self.assertTrue(file_check1,msg = "True")
